...
이 회귀는 ROB, issue queue/global arbiter, multiplier, branch predictor, target/loop block buffer, AXI outbound/inbound bridge, I/D fabric, SoC peripheral path, PLIC, CLINT 13종을 검증합니다. AXI 검사는 무응답 watchdog/late-response drain뿐 아니라 4 KiB 및 local-window 경계를 넘는 burst가 memory side effect 없이 오류로 끝나는지도 포함합니다.
