...
Icarus 회귀는 decoder/C expander, divider, RV32F 실행기, fetch queue, target/loop block buffer, LSU AGU, LSQ ordering/tombstone/device-store path, store buffer, writeback/recovery buffer, CSR/privilege/trap 상태 전이와 PMP mode/priority/permission을 검증합니다. FPU 회귀는 `scripts/gen_fpu_diff_vectors.py`가 exact rational arithmetic으로 생성한 3,870개 vector의 result bit와 `fflags`도 비교하며, `python scripts/gen_fpu_diff_vectors.py --check`로 checked-in fixture의 재현성을 확인할 수 있습니다.
