...
ITIM/DTIM에 적재됩니다. 기본으로 같은 Host AXI 경로를 통해 모든 최종 load byte와
BSS zero-fill을 다시 읽어 ELF와 exact-compare하며, 이 검증이 PASS한 뒤에만 Boot
entry와 CLINT MSIP를 기록해 코어를 깨웁니다. Boot ROM은 이후 ELF entry로
jump합니다. 자세한 파일 구성과
...
Icarus 회귀는 decoder/C expander, divider, RV32F 실행기, fetch queue, target/loop block buffer, LSU AGU, LSQ ordering/tombstone/device-store path, store buffer, writeback/recovery buffer, CSR/privilege/trap 상태 전이와 PMP mode/priority/permission을 검증합니다. FPU 회귀는 `scripts/gen_fpu_diff_vectors.py`가 exact rational arithmetic으로 생성한 6,470개 vector의 result bit와 `fflags`도 비교하며, `python scripts/gen_fpu_diff_vectors.py --check`로 checked-in fixture의 재현성을 확인할 수 있습니다.
