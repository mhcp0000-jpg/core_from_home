...
MODULES = [
    ...
    m("rv_backend", "A. Top-level integration", "rtl/backend/rv_backend.sv",
      ...
      ("Decode/Rename/ROB/IQ", "5-port issue와 execution", "4-source completion/2-wide commit"),
    ...
    m("rv_branch_predictor", "B. Frontend", "rtl/frontend/rv_branch_predictor.sv",
      ...
      ("BTB 256×4 lookup", "bimodal/gshare/chooser", "speculative+committed GHR/RAS"),
    ...
    m("rv_rename2", "C. Rename and scheduling", "rtl/backend/rv_rename2.sv",
      ...
      ("RAT/RRAT lookup", "INT/FP free-list allocation", "same-pair bypass/checkpoint"),
    ...
    m("rv_issue_queue", "C. Rename and scheduling", "rtl/backend/rv_issue_queue.sv",
      ...
      ("56-entry unified payload", "per-source ready scoreboard", "wrap-aware oldest-ready select"),
      "두 candidate와 occupancy",
      "dispatch edge 뒤 candidate가 보이고 WB broadcast는 dependent ready를 edge에서 갱신한다.",
    ...
    m("rv_fpu", "D. Execute and writeback", "rtl/backend/rv_fpu.sv",
      ...
      ("exact bit-level execute", "elastic stage chain", "result+fflags hold"),
      "FP/INT result, fflags, exception identity",
      "기본 LATENCY=3으로 accept 후 세 pipeline edge 뒤 result valid가 된다.",
    ...
    m("rv_writeback_arbiter", "D. Execute and writeback", "rtl/backend/rv_writeback_arbiter.sv",
      ...
      "순수 조합 arbitration이며 선택 결과가 같은 edge에 producer/ROB/PRF handshake된다.",
    ...
    m("rv_lsq", "E. Load/store subsystem", "rtl/backend/rv_lsq.sv",
      ...
      ("LQ24/SQ16 arrays", "ordering/forward candidates", "commit/free/tombstone recovery"),
    ...
    m("rv_pmp", "F. Privilege, trap and protection", "rtl/backend/rv_pmp.sv",
      ...
      ("entry range decode", "lowest-index first-match", "containment/permission check"),
